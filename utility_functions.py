...
def smiles_to_xyz(smiles):
    ...
    func_mol = Chem.MolToMolBlock(m4).splitlines()
    atom_atomic_number_dict = {"C":6, "N":7, "O":8, "Cl":17, 'S':16}
    atomic_numbers_fg = []
    coordinates_fg = []
    for atom in range(4, 4 + m4.GetNumAtoms()):
        jj = ' '.join((func_mol[atom].split()))
        coord = jj.split()
        # coordinates print out
        atomic_numbers_fg.append(atom_atomic_number_dict[coord[3]])
        coordinates_fg.append([float(coord[0]), float(coord[1]), float(coord[2])])
    return atomic_numbers_fg,coordinates_fg

def centeroidnp(arr):
    length = arr.shape[0]
    sum_x = torch.sum(arr[:, 0])
    sum_y = torch.sum(arr[:, 1])
    sum_z = torch.sum(arr[:, 2])
    return torch.FloatTensor([sum_x/length, sum_y/length, sum_z/length])
...
def collate_atoms(mol_dicts,
    ...
    Args:
        ...
        all_types (list of int, optional): list of all atom types in the data set in
            ...
            [1, 6, 7, 8, 9, 16, 17, 18], which are all atomic charges in QM9 and 10 as stop
            token)
...
def generate_molecules(amount,
    ...
    atoms_unfinished[:, [1]] = 0  # mark origin token as finished
    ...
    def build_batch(i):
        ...
        return batch
    #logging.info(positions[:, 0:5])
    #logging.info(atom_numbers[:,0:7])
    #logging.info(n_tokens+len(atomic_numbers_fg))
    for i in range(n_tokens+len(atomic_numbers_fg), max_length):
        ...
        while torch.sum(unfinished) > 0:
            ...
            batch = build_batch(i)
            #logging.info("Inserting atom "+str(i)+" "+str(positions[:, 0:i]))
            #logging.info(atom_numbers[:,0:7])
            #logging.info(n_tokens+len(atomic_numbers_fg))
            ...
            unfinished[unfinished.clone()] = pred_stop & ~finished[unfinished]
        ...
        if idx > 0 and torch.sum(finished) > 0:
            ...
            logging.info(positions[finished, :i]) 
        # mark finished moleclues in global unfinished mask
        global_unfinished[global_unfinished.clone()] = ~finished[global_unfinished]
    ...
    for i in range(len(atom_numbers)):
        logging.info("atom_numbers "+str(len(atom_numbers[i]))+" "+str(atom_numbers[i]))
    if failed_counter > 0:
